...
## A Fork with more features

This is a fork by Brandon Kalinowski. It adds a few features:

1. Expanded comment rule to enable `cov:include` rule. This makes a line marked as missed as hit. This is useful for false negatives (which is especially common with testing interactive CLI binaries).
2. The derive feature is disabled by default but can be manually enabled. Derive can be inaccurate for branch coverage, but grcov doesn't appear to have this issue with line-coverage reports.
3. Fixed CLI message.
4. Fixed lint errors.
5. Optimized release binary for size.

This utility is designed as part of my rust coverage collector container image.
...
Copyright © 2020 [brandonkal](https://github.com/brandonkal).
